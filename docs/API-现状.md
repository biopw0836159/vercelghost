...
4. **串流回应绕过闸道超时**：Railway 闸道 300 秒没资料就切断（实测查彩种要 8 分钟，
   非串流必定 502 `upstream error`）。加 `?stream=1` 改回 NDJSON，
   每翻一页送一则 `{type:'progress'}`，连线持续有资料就不会被切。

线上实测（HS 平台 8/12 查「排列三五」）：**497 秒、翻满 125 页、
源头 621,324 笔精确筛出 852 笔、`truncated=false`**。
