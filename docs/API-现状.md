...
### 1. 日期边界在当地时间 11:00，不是 00:00

同一个会员（`hmhm777`）用两种查法比对，差异出在早班：

| 查法 | 8/13 早班笔数 | 该班起始时间 |
|---|---|---|
| 查 `8/13 ~ 8/13` | 158 | 10:57:44 |
| 查 `8/12 ~ 8/13` | **230** | **08:00:57** |

单日查漏了 72 笔。后端的 `dateStart=D` 实际是「D 当地时间 11:00 → D+1 11:00」。
...
7. **`dateStart`/`dateEnd` 一天的边界是几点、哪个时区**？实测在当地时间 11:00 左右，
