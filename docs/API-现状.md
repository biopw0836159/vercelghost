...
| `/api/open/external-game-stats?platform=ALL` | GET | ✅ 可用，本站已接 | 179 笔（18 平台 × 34 种游戏） |
...
### 新进会员 `POST /api/v1/new-members`

14 个栏位：平台 / 会员账号 / 团队人数 / 上级 / 余额 / **奖金（奖金号）** / 注册时间 /
**首充日期·金额·管道** / 最后登录 / **最后登录 IP** / 来源 / **注册码**。

实测限制（2026-08-15）：

- **`platforms` 必须列举，传 `['ALL']` 会回 0 笔**
- `username` / `filterType` / `regUrl` 这几个参数**实际无效**（带了跟没带结果一样）
- 没有分页也没有笔数上限，一次全回（ALL 平台 13 天实测 1307 笔 / 数秒）
- 查一天会涵盖到隔天 —— 业务日 03:00 切点的正常表现

本站做成 C 引擎「新进会员」，把帐号按六个维度归组：
**同上级 / 同奖金号 / 同注册码 / 同首充金额 / 同最后登入 IP / 30 分钟内密集创号**。
只陈述「哪些帐号的某个值相同」，不下结论；同 IP 另外分出机房出口。

### 外接游戏 `GET /api/open/external-game-stats`

栏位：平台 / 彩种（游戏名）/ 人数 / 投注金额 / 返点 / 奖金 / 盈亏。
对应后台「报表查询 → 外接统计」，与彩票是两套资料，**不能混算**。

三个后端口径问题（都照原样保留，不自行修正，否则会跟后台对不起来）：

1. **`ALL` 与单平台查询结果不同** —— `ALL` 里的 HS 有 14 笔（多一个 `BTC/USDT`，投注 90），
   单查 HS 只有 13 笔。连查三次单平台都稳定，不是随机波动。
2. **盈亏算法不一致** —— 多数是「投注−奖金−返点」，但 `VR` 那笔是「投注−奖金」
   （差额正好等于返点 13.965）→ **一律采用后端给的盈亏值，不自己算**。
3. **人数栏位不完整** —— 179 笔里只有 40 笔有人数，138 笔有投注却是 0 人
   → 照原值呈现，介面标注「多数为 0」，不要拿来算人均。
